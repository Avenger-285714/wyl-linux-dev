...
/*
 * sys_ni_syscall() is declared inside the !ARCH_HAS_SYSCALL_WRAPPER
 * guard in include/linux/syscalls.h (see the #ifndef block around
 * asmlinkage long sys_ni_syscall(void)), so it is not visible with
 * the wrapper enabled.  Provide a local forward declaration, matching
 * what arm64 does in arch/arm64/kernel/syscall.c.
 */
