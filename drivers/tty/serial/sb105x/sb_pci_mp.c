 ...
 * Copyright (C) 2017 Huacai Chen <chenhuacai@kernel.org>
 * Copyright (C) 2024 Mingcong Bai <jeffbai@aosc.io>
...
static _INLINE_ unsigned int read_option_register(struct mp_port *mtpt,
						  int offset);
...
static inline void mp_update_mctrl(struct sb_uart_port *port, unsigned int set,
				   unsigned int clear);
static int mp_startup(struct sb_uart_state *state, int init_hw);
static void mp_shutdown(struct sb_uart_state *state);
static void mp_change_speed(struct sb_uart_state *state,
			    const struct MP_TERMIOS *old_termios);

static inline int __mp_put_char(struct sb_uart_port *port,
				struct circ_buf *circ, unsigned char c);
static int mp_put_char(struct tty_struct *tty, unsigned char ch);

static void mp_put_chars(struct tty_struct *tty);
static ssize_t mp_write(struct tty_struct *tty, const u8 *buf, size_t count);
...
static int mp_get_info(struct sb_uart_state *state,
		       struct serial_struct *retinfo);
static int mp_set_info(struct sb_uart_state *state,
...
static int mp_tiocmset(struct tty_struct *tty, unsigned int set,
		       unsigned int clear);
...
static int mp_get_count(struct sb_uart_state *state,
			struct serial_icounter_struct *icnt);
static int mp_ioctl(struct tty_struct *tty, unsigned int cmd,
		    unsigned long arg);
static void mp_set_termios(struct tty_struct *tty,
			   const struct MP_TERMIOS *old_termios);
...
static inline void mp_report_port(struct uart_driver *drv,
				  struct sb_uart_port *port);
static void mp_configure_port(struct uart_driver *drv,
			      struct sb_uart_state *state,
			      struct sb_uart_port *port);
static void mp_unconfigure_port(struct uart_driver *drv,
				struct sb_uart_state *state);
...
static int mp_remove_one_port(struct uart_driver *drv,
			      struct sb_uart_port *port);
...
static unsigned int multi_get_divisor(struct sb_uart_port *port,
				      unsigned int baud);
static void multi_set_termios(struct sb_uart_port *port,
			      struct MP_TERMIOS *termios,
			      const struct MP_TERMIOS *old);
static void multi_pm(struct sb_uart_port *port, unsigned int state,
...
static int multi_verify_port(struct sb_uart_port *port,
...
static int sb1054_get_register(struct sb_uart_port *port, int page, int reg)
{
	...
	if (page <= 0) {
		pr_err("sb105x: page 0 can not use this function.\n");
		return -1;
	}
	...
	case 4:
		...
		break;
	default:
		pr_err("sb105x: error invalid page number.\n");
		return -1;
	}
	...
	if (state->info->flags & UIF_INITIALIZED) {
		if (((old_flags ^ port->flags) & UPF_SPD_MASK) ||
			...
			if (port->flags & UPF_SPD_MASK) {
				pr_notice("%s sets custom speed on ttyMP%d."
				       " This is deprecated.\n",
...
static int mp_ioctl(struct tty_struct *tty, unsigned int cmd, unsigned long arg)
{
	...
	case SETDEEPFIFO:
		...
		return ret;
	case SETTTR:
		if (info->port.type == PORT_16C105X ||
		    info->port.type == PORT_16C105XA) {
	...
	case TIOCGGETINTERFACE:
		if (mp_devs[arg].revision == 0xc0) {
			...
			return (inb(mp_devs[arg].option_reg_addr +
				    MP_OPTR_IIR0 + (state->port->line / 8)));
		}
...
static void mp_close(struct tty_struct *tty, struct file *filp)
{
	...
	if ((tty->count == 1) && (state->count != 1)) {
		pr_warn("%s: bad serial port count; tty->count is 1, "
		       "state->count is %d\n",
		       __func__, state->count);
		state->count = 1;
	}
	if (--state->count < 0) {
		pr_warn("%s: bad serial port count for ttyMP%d: %d\n",
			__func__, port->line, state->count);
		state->count = 0;
	}
	if (state->count)
		goto done;
				  ...
				  struct sb_uart_port *port)
{
	...
	case UPIO_PORT:
		...
		break;
	case UPIO_HUB6:
		snprintf(address, sizeof(address), "I/O 0x%x offset 0x%x",
...
static const struct tty_operations sb_mp_ops = {
	.open = mp_open,
	.close = mp_close,
	.write = mp_write,
...
static int mp_register_driver(struct uart_driver *drv)
{
	...
	drv->state =
		kmalloc(sizeof(struct sb_uart_state) * drv->nr, GFP_KERNEL);
	retval = -ENOMEM;
	if (!drv->state) {
...
static void autoconfig(struct mp_port *mtpt, unsigned int probeflags)
{
	...
	DEBUG_AUTOCONF("ttyMP%d: autoconf (0x%04x, 0x%p): ", mtpt->port.line,
		       mtpt->port.iobase, mtpt->port.membase);
	...
	if (mtpt->device->nr_ports >= 8)
		b_ret = read_option_register(
			mtpt, (MP_OPTR_DIR0 + ((mtpt->port.line) / 8)));
	else
...
static _INLINE_ void receive_chars(struct mp_port *mtpt, int *status)
{
	...
	do {
		if ((lsr & UART_LSR_PE) &&
		    (mtpt->port.mdmode & MDMODE_ENABLE)) {
			...
			tty_insert_flip_char(&tt_port[mtpt->port.line], ch,
					     flag);
		} else {
			ch = serial_inp(mtpt, UART_RX);
...
static inline void multi_handle_port(struct mp_port *mtpt)
{
	...
	if (status & UART_LSR_THRE) {
		if ((mtpt->port.type == PORT_16C105X) ||
			...
			if (mtpt->interface >= RS485NE) {
				while ((status = serial_in(mtpt, UART_LSR) &
						 0x60) != 0x60)
					;
...
static irqreturn_t multi_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if (lhead == iinfo->head && pass_counter++ > PASS_LIMIT) {
			pr_err("multi: too much work for irq%d\n", irq);
			break;
		}
	} while (lhead != end);
...
static int serial_link_irq_chain(struct mp_port *mtpt)
{
	...
	if (i->head) {
		...
		ret = request_irq(mtpt->port.irq, multi_interrupt, irq_flags,
				  "serial", i);
		if (ret < 0)
...
static int multi_startup(struct sb_uart_port *port)
{
	...
	if (!(mtpt->port.flags & UPF_BUGGY_UART) &&
		...
		pr_notice("ttyS%d: LSR safety check engaged!\n", mtpt->port.line);
		//return -ENODEV;
	}

	if ((!is_real_interrupt(mtpt->port.irq)) ||
	    (mtpt->poll_type == TYPE_POLL)) {
				      ...
				      unsigned int baud)
{
	...
	if ((port->flags & UPF_MAGIC_MULTIPLIER) && baud == (port->uartclk / 4))
		quot = 0x8001;
			     ...
			     struct serial_struct *ser)
{
	if (ser->irq >= NR_IRQS || ser->irq < 0 || ser->baud_base < 9600 ||
	    ser->type < PORT_UNKNOWN || ser->type == PORT_STARTECH)
		return -EINVAL;
	return 0;
}
...
static void __init multi_init_ports(void)
{
	...
	for (k = 0; k < NR_BOARD; k++) {
		...
		for (i = 0; i < sbdev->nr_ports; i++, mtpt++) {
			...
			if (((sbdev->device_id == PCI_DEVICE_ID_MP4) &&
			     (sbdev->revision == 0x91)))
				mtpt->interface_config_addr =
					sbdev->option_reg_addr + 0x08 + i;
			else if (sbdev->revision == 0xc0)
				mtpt->interface_config_addr =
					sbdev->option_reg_addr + 0x08 +
					(i & 0x1);
			else
				mtpt->interface_config_addr =
					sbdev->option_reg_addr + 0x08 + i / 8;
			...
			osc = inb(sbdev->option_reg_addr + MP_OPTR_DIR0 +
				  i / 8) &
			      0x0F;
			if (osc == 0x0f)
				osc = 0;
...
static int init_mp_dev(struct pci_dev *pcidev, mppcibrd_t brd)
{
	...
	case PCIE_DEVICE_ID_GT_MP32: {
		...
		sbdev->nr_ports =
			((portnum_hex / 16) * 10) + (portnum_hex % 16);
	} break;
