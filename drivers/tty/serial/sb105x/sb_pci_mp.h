...
#ifdef MP_DEBUG
#define DEBUG_AUTOCONF(fmt...) pr_debug(fmt)
#else
...
#ifdef MP_DEBUG
#define DEBUG_INTR(fmt...) pr_debug(fmt)
#else
...
struct mp_port {
	...
	void (*pm)(struct sb_uart_port *port, unsigned int state,
		   unsigned int old);
