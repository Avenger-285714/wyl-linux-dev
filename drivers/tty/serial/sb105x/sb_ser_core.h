...
#define UART_ENABLE_MS(port, cflag)                             \
	((port)->flags & UPF_HARDPPS_CD || (cflag) & CRTSCTS || \
	 !((cflag) & CLOCAL))
...
static void sb_uart_update_timeout(struct sb_uart_port *port,
...
static unsigned int sb_uart_get_baud_rate(struct sb_uart_port *port,
...
static unsigned int sb_uart_get_divisor(struct sb_uart_port *port,
					unsigned int baud)
{
	unsigned int quot;
...
static inline void sb_uart_handle_dcd_change(struct sb_uart_port *port,
					     unsigned int status)
{
	struct sb_uart_info *info = port->info;
...
static inline void sb_uart_handle_cts_change(struct sb_uart_port *port,
					     unsigned int status)
{
	struct sb_uart_info *info = port->info;
